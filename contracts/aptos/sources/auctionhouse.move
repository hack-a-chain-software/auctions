module auctionhouse::AuctionHouse {
    ...
    /// Auction representation to be returned to frontend
    struct ConsumableAuction has drop, copy {
        id: u64,
        creator: address,
        start_time: u64,
        end_time: u64,
    ...
    /// Generates a ConsumableAuction based on a reference to an Auction
    fun to_consumable_auction(id: u64, auction: &Auction): ConsumableAuction {
        ConsumableAuction {
            id,
            creator: auction.creator,
            start_time: auction.start_time,
            end_time: auction.end_time,
            auction_coin: auction.auction_coin,
            min_selling_price: auction.min_selling_price,
            min_increment: auction.min_increment,
            current_bid: auction.current_bid,
            current_bidder: auction.current_bidder,
            locked_token_id: auction.locked_token_id,
            coins_claimed: auction.coins_claimed,
        }
    }
    ...
    /// VIEW METHODS
    public entry fun is_user_authorized(auction_house_address: address, user: address): bool acquires AuctionHouse {
        let auction_house = borrow_global<AuctionHouse>(auction_house_address);
        if (option::is_none(&auction_house.allowed_users)) {
            true
        } else {
            table_set::contains(option::borrow(&auction_house.allowed_users), user)
        }
    }

    public entry fun get_auctions_len(auction_house_address: address): u64 acquires AuctionHouse {
        let auction_house = borrow_global<AuctionHouse>(auction_house_address);
