...
export async function testAptosCoin(
  ...
  const auctionHouseClient = new AuctionHouseClient(
    NODE_URL,
    moduleAddress.address().hex(),
    ownerAccount.address().hex()
  );
  ...
  sh.exec(`
        aptos move compile --named-addresses auctionhouse=${moduleAddress
          .address()
          .hex()} --included-artifacts all --save-metadata
    `);

  const modulePath = path.join(__dirname, "..");
  const packageMetadata = fs.readFileSync(
    path.join(modulePath, "build", "AuctionHouse", "package-metadata.bcs")
  );
  ...
  const moduleData3 = fs.readFileSync(
    path.join(
      modulePath,
      "build",
      "AuctionHouse",
  ...
  let txnHash = await client.publishPackage(
    ...
    [
      ...
      new TxnBuilderTypes.Module(
        new HexString(moduleData3.toString("hex")).toUint8Array()
      ),
    ]
  );
  ...
  await client.waitForTransaction(
    await auctionHouseClient.addAuthorizedUser(
      ownerAccount,
      bidder1.address().hex()
    ),
    { checkSuccess: true }
  ...
  await client.waitForTransaction(
    await auctionHouseClient.addAuthorizedCollection(
      ownerAccount,
      nftCollection
    ),
    { checkSuccess: true }
  ...
  const createdByUserLen = await auctionHouseClient.getCreatedByUserAuctionsLen(
    ownerAccount.address().hex()
  );
  assert(createdByUserLen == 1);

  const createdByUserLen2 =
    await auctionHouseClient.getCreatedByUserAuctionsLen(
      bidder1.address().hex()
    );
  assert(createdByUserLen2 == 0);

  const createdByUser = auctionHouseClient.getCreatedByUserAuctions(
    ownerAccount.address().hex(),
    0,
    10
  );
  ...
  // bid
  await client.waitForTransaction(
    await auctionHouseClient.bid(bidder1, "0", "10000", aptosCoin),
    { checkSuccess: true }
  );

  const bidByUserLen = await auctionHouseClient.getBidByUserAuctionsLen(
    bidder1.address().hex()
  );
  assert(bidByUserLen == 1);
  const bidByUser = await auctionHouseClient.getBidByUserAuctions(
    bidder1.address().hex(),
    0,
    10
  );
